...
import com.socialmediasaver.status.util.ViewPagerFragment;

import java.util.ArrayList;


public class HomeBannersPagerAdapter extends FragmentStatePagerAdapter {
    public static int LOOPS_COUNT = 1000;
    private ArrayList<WhatsappStatusModel> sliderListResponsesData;


    public HomeBannersPagerAdapter(FragmentManager manager, ArrayList<WhatsappStatusModel> sliderListResponsesData) {
        super(manager);
    ...
    public Fragment getItem(int position) {
        if (sliderListResponsesData != null && sliderListResponsesData.size() > 0) {
           // position = position % sliderListResponsesData.size(); // use modulo for infinite cycling
            return ViewPagerFragment.newInstance(sliderListResponsesData.get(position),sliderListResponsesData,position);
        } else {
            return ViewPagerFragment.newInstance(null, sliderListResponsesData, position);
        }
    }
