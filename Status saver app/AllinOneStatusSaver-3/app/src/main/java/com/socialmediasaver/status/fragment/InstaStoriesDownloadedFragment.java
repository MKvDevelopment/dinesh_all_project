...
import static com.socialmediasaver.status.util.Utils.RootDirectoryDownloadInstaStories;
...
public class InstaStoriesDownloadedFragment extends Fragment implements FileListClickInterface {
    private FragmentHistoryBinding binding;
    private FileListAdapter fileListAdapter;
    private ArrayList<File> fileArrayList;
    private InstagramFolderActivity activity;
    public static InstaStoriesDownloadedFragment newInstance(String param1) {
        InstaStoriesDownloadedFragment fragment = new InstaStoriesDownloadedFragment();
    ...
    private void getAllFiles(){
        ...
        File[] files = RootDirectoryDownloadInstaStories.listFiles();
        if (files!=null) {
            for (File file : files) {
                fileArrayList.add(file);
            }
            fileListAdapter = new FileListAdapter(activity, fileArrayList, InstaStoriesDownloadedFragment.this);
