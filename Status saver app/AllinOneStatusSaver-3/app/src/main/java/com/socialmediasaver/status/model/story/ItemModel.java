package com.socialmediasaver.status.model.story;
...
public class ItemModel extends MyStory implements Serializable, Parcelable {
    ...
    protected ItemModel(Parcel in) {
        taken_at = in.readLong();
        pk = in.readLong();
        id = in.readString();
        device_timestamp = in.readLong();
        media_type = in.readInt();
        code = in.readString();
        client_cache_key = in.readString();
        filter_type = in.readInt();
        original_width = in.readInt();
        original_height = in.readInt();
        has_audio = in.readByte() != 0;
        video_duration = in.readDouble();
        caption_is_edited = in.readByte() != 0;
        caption_position = in.readInt();
        is_reel_media = in.readByte() != 0;
        photo_of_you = in.readByte() != 0;
        organic_tracking_token = in.readString();
        expiring_at = in.readLong();
        can_reshare = in.readByte() != 0;
        can_reply = in.readByte() != 0;
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeLong(taken_at);
        dest.writeLong(pk);
        dest.writeString(id);
        dest.writeLong(device_timestamp);
        dest.writeInt(media_type);
        dest.writeString(code);
        dest.writeString(client_cache_key);
        dest.writeInt(filter_type);
        dest.writeInt(original_width);
        dest.writeInt(original_height);
        dest.writeByte((byte) (has_audio ? 1 : 0));
        dest.writeDouble(video_duration);
        dest.writeByte((byte) (caption_is_edited ? 1 : 0));
        dest.writeInt(caption_position);
        dest.writeByte((byte) (is_reel_media ? 1 : 0));
        dest.writeByte((byte) (photo_of_you ? 1 : 0));
        dest.writeString(organic_tracking_token);
        dest.writeLong(expiring_at);
        dest.writeByte((byte) (can_reshare ? 1 : 0));
        dest.writeByte((byte) (can_reply ? 1 : 0));
    }
}
