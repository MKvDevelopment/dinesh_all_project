...
import com.typingwork.jobathome.Activity.MainActivity;
...
public class Constant {
    ...
    public static String randomRobotCodeGererate(String plan) {
        //Captcha code for user input

        String LETTERS = "abcdefghijkmnopqrstuvwxy";
        String SYMBOL = "Ilk&%{@£:,-!/'$¥~>`^§]¢©";

        String PaidLetters = "£I&I?-_¡%Il:'.,/I0*@l$€I¥¢©®™0~kl¿¡^KIO¬°§l¶×lOI|`¡|¦I\\÷l;`[Il]{}<I>0";

        char[] Free_Plan_CODE = (("0123456789") + SYMBOL + LETTERS.toLowerCase() + LETTERS.toUpperCase()).toCharArray();
        char[] Paid_Plan_CODE = PaidLetters.toCharArray();
        ...
        if (plan.equals("free")){
            ...
            for (int i = 0; i < 20; i++) {
                result.append(Paid_Plan_CODE[new Random().nextInt(Paid_Plan_CODE.length)]);
            }
            return result.toString();
        }
   ...
   /*     Button nbutton = alert.getButton(AlertDialog.BUTTON_NEGATIVE);
    ...
    public static void CheckAppUpdate(MainActivity activity) throws PackageManager.NameNotFoundException {

        String app_version = String.valueOf(activity.getPackageManager().getPackageInfo(activity.getPackageName(), 0).versionCode);

        if (!app_version.contains(MainActivity.FIREBASE_VERSION2)) {

            AlertDialog alert = new AlertDialog.Builder(activity).setPositiveButton("Update Now", (dialogInterface, i) -> {
